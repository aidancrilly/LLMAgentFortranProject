...
- `ReadFile` streams numbered slices (default 400 lines) from any repository file, ensuring paths stay inside the project root. Use it for quoting code and cite the lines it reports. Prefer the tools above.
...
- `GitReplaceBlock` applies changes by specifying start and end lines to replace as well as the replacement text and writes a `<file>.orig` backup before touching the original. After editing, run `GitDiff` to verify the applied hunks.
